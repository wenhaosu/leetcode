...
public:
    ...
    bool isValidRow(vector<vector<char>> &board, int row) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (status[board[row][i] - '0']) return false;
            status[board[row][i] - '0'] = true;
        }
        return true;
    }
    ...
    bool isValidCol(vector<vector<char>> &board, int col) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (status[board[i][col] - '0']) return false;
            status[board[i][col] - '0'] = true;
        }
        return true;
    }
    ...
    bool isValidArea(vector<vector<char>> &board, int row, int col) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (status[board[row + i][col + j] - '0']) return false;
                status[board[row + i][col + j] - '0'] = true;
            }
        }
        return true;
    }
